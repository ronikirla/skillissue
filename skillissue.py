...
def main():
    ...
    parser.add_argument("-w", "--weight",
                        ...
                        help="Weight decay factor. The lower the values, the more your recent times are prioritized. Values between [0,1].\
                              Default 15/16.",
                        default=15/16)
    parser.add_argument("-m", "--min_weight",
                        type=restricted_float,
                        help="Increase this to drop old attempts completely. After the weight of a time has decayed to this value, the time\
                              will be dropped. Useful with -H. Default 0.",
                        default=0)
    parser.add_argument("-d", "--drop_missing",
                        action="store_true",
                        help="By default, resets are counted as infinite time runs, essentially treating them as forfeits.\
                              This option ignores them instead.")
    ...
    parser.add_argument("-H", "--hist", "--histogram",
                        action="store_true",
                        help="Output a histogram visualizing the distribution of your times instead of a progression chart.\
                              Forces -d and ignores -p and -a.")
    ...
    parser.add_argument("-P", "--per_split",
                        action="store_true",
                        help="Generate the output splitwise instead of based on the finish times. Stores the splitwise plots in ./skillissue_output\
                              and shows an aggregate plot of all splits.")
